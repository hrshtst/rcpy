...
OUTPUT_DIR="benchmarks/results/benchmark_run_fair_${TIMESTAMP}"
...
for i in $(seq 1 $NUM_RUNS); do
  ...
  for size in "${SIZES[@]}"; do
    ...
    echo "  Running: NumPy (Power iterataion)"
    ...
    uv run python "$MAIN_SCRIPT" experiment.use_numpy_version=false taichi.backend=gpu esn.use_taichi_init=true solver.use_taichi_ridge=true experiment.use_numpy_predict_in_taichi=true $COMMON_ARGS
  done
done
